...
def _extract_argmax_and_embed(embedding,
  ...
  def loop_function(prev, _):
    ...
    if output_projection is not None:
      prev = nn_ops.xw_plus_b(prev, output_projection[0], output_projection[1])#prev:16x491
    prev_symbol = math_ops.argmax(prev, 1)
    # Note that gradients will not propagate through the second parameter of
    # embedding_lookup.
    emb_prev = embedding_ops.embedding_lookup(embedding, prev_symbol)#从embedding矩阵查找,embedding:491x100,prev_symbol:16
...
def attention_decoder(initial_state,#(c,h)  c.shape/h.shape:(16, 512)
    ...
    with variable_scope.variable_scope(scope or "attention_decoder", dtype=dtype) as scope:
        ...
        def body(time_step, prev_o_t, prev_softmax_input, state_c, state_h, outputs2):#prev_o_t=prev:shape=(16,512) 
                                                #outputs:shape=(16, ?, 491) prev_softmax_input=init_word:shape=(16, 491)
            state = tf.nn.rnn_cell.LSTMStateTuple(state_c,state_h)#第一次随机初始状态，之后调用之前的
            with variable_scope.variable_scope("loop_function", reuse=True):
                inp = loop_function(prev_softmax_input, time_step)#shape=(16,100)

            input_size = inp.get_shape().with_rank(2)[1]#100
            if input_size.value is None:
                raise ValueError("Could not infer input size from input: %s" % inp.name)
            x = tf.concat([inp,prev_o_t],1)#shape=(16, 612) 　这个地方inp ,prev_o_t = loop_function(softmax_output),output
            # Run the RNN.
            ...
            with variable_scope.variable_scope("AttnOutputProjection"):
                ...
                with variable_scope.variable_scope("FinalSoftmax"):
                    softmax_input = linear(output,vocab_size,False)#shape=(16, 491) #decoder层后加一层softmax??作为softmax_input

            new_outputs = tf.concat([outputs2,tf.expand_dims(softmax_input,1)],1)#shape=(16, ?, 491)[,...y_t-1,y_t,...]
...
# START keyword is 0
        init_word = np.zeros([batch_size, vocab_size])#shape=(16,491)
        ...
        outputs = tf.while_loop(cond, body, loop_vars, shape_invariants)##shape=(16, ?, 491)
        '''
        loop_vars = [...]
...
def embedding_attention_decoder(initial_state,#(c,h)
    ...
    with variable_scope.variable_scope(scope or "embedding_attention_decoder", dtype=dtype) as scope:
        embedding = variable_scope.get_variable("embedding",[num_symbols, embedding_size])#shape=(491,100)
        loop_function = _extract_argmax_and_embed(embedding,
                        ...
                        #(16,491)==>(16,100)找argmax,然后embedding
